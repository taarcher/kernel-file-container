...
//    if(strcmp("/", path)!=0)
    ...
    cid = fcontainer_getcid(FCFS_DATA->devfd, fuse_get_context()->pid); 
    
    //what to do if the container is not created yet
    //not sure what to do since a cid is not given.
    if (cid == -1){
        cid = 0;
...
//	    FCFS_DATA->rootdir, path, fpath);
    ...
    sprintf( container, "/%d", cid );
    strncat( fpath, container, PATH_MAX);
...
int fcfuse_getattr(const char *path, struct stat *stbuf)
{
    ...
    //added to get container id  --- determined it isn't needed
    /**
    int cid;
    int pid;
    ...
    pid = current->pid;
    cid = fcontainer_getcid(int devfd, int pid);
    **/
    // end of added
    printf( "path: %s", path);
...
//        return log_syscall("mkdir", mkdir(fpath, mode), 0);
    ...
    log_msg("\nfcfuse_mknod(path=\"%s\", mode=0%3o, dev=%lld)\n",
	  path, mode, FCFS_DATA->devfd);
    
//not needed because it is handled in the function
//    fcfuse_fullpath(fpath, path);

//commented out to try calling mkpath in the return function
//    return log_syscall("mkdir", mkdir(fpath, mode), 0);

    return log_syscall(" mkdir", mkpath( fpath, mode), 0);
    **/
}
...
//added
/**
* I did not write this code, but found it posted in an online message board. As it is 
* published on the message board, public use is assumed on my part.
*
* It is supposed to check a file patch and recurrisively create the path as needed.
*
* Found here: https://stackoverflow.com/questions/2336242/recursive-mkdir-system-call-on-unix
**/
int mkpath(char* file_path, mode_t mode) {
  ...
  log_msg("    fcfuse_mkpath:  rootdir = \"%s\", path created= \"%s\"\n",
	    FCFS_DATA->rootdir, file_path);
  return 0;
}
